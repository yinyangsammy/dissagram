...
| `requirements.txt` broke Heroku deployment | The file had been generated in UTF-16LE via PowerShell's `>` redirect operator, which pip cannot read | Re-saved as UTF-8; documented the safe alternative: `pip freeze \| Out-File -Encoding utf8 requirements.txt` |
...
| Stripe webhook returned `500` on `checkout.session.completed` | Order creation and email sending errors were not safely separated — an email failure could crash the whole webhook response | Split order creation and email sending into separate `try/except` blocks so email failure can never block order creation |
| Stripe webhook metadata could not be read via `.get()` | Stripe's `StripeObject` type does not support standard Python `.get()` on nested objects, causing intermittent `KeyError` failures | Switched to parsing the raw request body as plain JSON (`json.loads(payload)`) before reading metadata, bypassing the `StripeObject` entirely |
...
| Archetype carousels stopped rendering entirely | A multi-line JavaScript string literal inside `querySelectorAll()` produced a silent `SyntaxError` that halted the entire script | Collapsed the selector strings onto single lines |
| Premium diss categories never appeared | The `is_premium` flag was missing from the JSON payload built in `_archetype_json` | Added `is_premium`, derived from `RoastCategory.is_free`, to each diss line object |
| Pack level stayed at `0` after purchase | Package `display_order` values were still set to the default (`0`) in Django admin | Updated pack display orders: Diss Pack → `1`, Burn Pack → `2` |
| Premium lines appeared above standard lines on Diss Detail | SQLite sorts `NULL` values before populated values, so premium lines (no `roast_style`) sorted to the top | Explicitly ordered selected lines by `-category__is_free` then `display_order` in the `diss_detail` view |
| Mandatory allauth email verification blocked registration in development | Development emails were printed to the console, making browser-based email verification impossible for testing | Set `ACCOUNT_EMAIL_VERIFICATION = 'none'` for the submitted assessment build |
| Homepage CTA button was unstyled plain text | The `.hero-cta-main` CSS was defined only inside the How It Works template's `{% block extra_css %}`, not the homepage | Added the required CTA styles to the homepage template's own `{% block extra_css %}` block |
| Edit Diss did not restore the selected roast style visually | The hidden input value was restored on load, but the visible avatar card's selected state was not triggered | Added logic to find and click the matching style card during edit-form initialisation, restoring the visual state |
| Hero image characters appeared stretched and distorted | `background-size: 100% 100%` forces the image to fill both width and height of the container independently, stretching whichever dimension does not match | Changed to `background-size: cover` which scales the image proportionally, preserving aspect ratio at every screen size |
| Bootstrap JS trailing space in CDN URL broke the entire site | A stray space at the end of the Bootstrap JS CDN URL (`bootstrap.bundle.min.js `) caused the script to silently fail to load, breaking all navbar dropdowns, the burger menu and Bootstrap-dependent components site-wide | Removed the trailing space from the `<script>` `src` attribute |
| Font Awesome icons flashed invisible on page load after performance optimisation | Applying `media="print" onload="this.media='all'"` to defer Font Awesome loading caused a brief flash where icons were unstyled or invisible, making the UI look broken during load | Reverted Font Awesome to standard synchronous loading; the performance gain did not outweigh the visible UX degradation |
| Procfile and runtime.txt were missing | Heroku needed explicit startup and Python runtime files before it could build the application | Added `Procfile` (`web: gunicorn dissagram.wsgi`) and `runtime.txt` (`python-3.12.3`) to the project root |
| `NoReverseMatch: 'dissers' is not a registered namespace` on homepage load | The `dissers` app URLs were included in `dissagram/urls.py` but `app_name` had not been set in `dissers/urls.py`, so Django could not resolve namespaced URL tags in `base.html` | Added `app_name = "dissers"` to `dissers/urls.py` and repeated for all apps; updated all `{% url %}` tags to use namespaced form e.g. `{% url 'dissers:disser_list' %}` |
| `SyntaxError: '[' was never closed` on Heroku deploy | A missing closing `]` in `INSTALLED_APPS` in `settings.py` caused Python to fail parsing the settings module entirely during `collectstatic` | Fixed the unclosed list in `settings.py`; noted that PowerShell copy-paste can silently drop characters |
| `MissingFileError` on Heroku deploy — `hero-image-hiptriphooray-about-m.webp` not found | `style.css` referenced a `.webp` image filename that did not exist in `static/images/` — the `-m` mobile variant had been added to CSS but the file was never created | Reverted the CSS reference to the existing `.png` source file; documented that Whitenoise's `CompressedManifestStaticFilesStorage` strictly validates every static file URL referenced in CSS at deploy time |
| Heroku build failed with `SyntaxError` in `settings.py` after Cloudinary config changes | An edit to the Cloudinary settings block introduced a syntax error that only surfaced during `collectstatic` on Heroku, not locally | Fixed syntax error and re-deployed; noted that `python manage.py check` locally does not always catch errors that surface during Heroku's build phase |
| Extra category tabs not saving on Heroku/PostgreSQL | Hidden category `<input>` elements used Django queryset index notation (`categories.1.id`) which assumes consistent return order; PostgreSQL does not guarantee ordering without an explicit `ORDER BY`, unlike SQLite | Passed a `category_map` dictionary keyed by lowercase category name from the view to the template; replaced index-based lookups with name-based lookups e.g. `{{ category_map.flavours }}` |
| Trip creation 500 error on Heroku with PostgreSQL | The formset was bound to an unsaved `Trip` instance (`temp_trip`) with no primary key; when items were saved, the foreign key pointed to a non-existent row — PostgreSQL enforces this strictly, SQLite ignores it silently | Removed `instance=temp_trip` from the POST formset binding; assigned `item.trip = trip` inside the save loop after the parent object had been committed |
| Cloudinary images not uploading on Heroku | `settings.py` read three separate Cloudinary environment variables but only `CLOUDINARY_URL` had been added to Heroku config vars | Added all three required config vars (`CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET`) to the Heroku dashboard alongside the existing `CLOUDINARY_URL` |
| Firefox 403 errors on OpenStreetMap map tiles | Firefox applies a stricter `Referrer-Policy` than Chrome, stripping the `Referer` header from tile requests; OSM tile servers rejected requests with no referrer | Added `<meta name="referrer" content="no-referrer-when-downgrade">` to `base.html` |
